...
/**
 * Recent Downloads Component - Display recently downloaded resources
 */
export const RecentDownloads: React.FC<RecentDownloadsProps> = ({
	...
	const {
		...
		queryKey: ['history-downloads'],
		queryFn: () => historyApi.getDownloads(),
	...
	// 3. Show ALL different files that have been downloaded, sorted by most recent download time
	const resources: Resource[] = useMemo(() => {
		...
		// Step 2: Keep only the most recent download for each file (resource.id)
		...
		// Result: All different files, sorted by most recent download time (newest first)
